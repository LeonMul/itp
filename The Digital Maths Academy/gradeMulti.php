...
<body>
        ...
        <?php
            ...
            if ($answer1 == "B") { $totalCorrectM++; }
            if ($answer2 == "C") { $totalCorrectM++; }
            if ($answer3 == "D") { $totalCorrectM++; }
            if ($answer4 == "B") { $totalCorrectM++; }
            if ($answer5 == "B") { $totalCorrectM++; }
            if ($answer6 == "A") { $totalCorrectM++; }
            if ($answer7 == "C") { $totalCorrectM++; }
            if ($answer8 == "C") { $totalCorrectM++; }
            if ($answer9 == "D") { $totalCorrectM++; }
            if ($answer10 == "B") { $totalCorrectM++; }
			...
			//Grab the current users score from the database and save it as a php variable ($dbUserScore)
					$sql = "SELECT score FROM multiplicationQuiz WHERE email='{$_SESSION['$myemail']}'";
			...
			if($dbUserScore<$totalCorrectM){
				$sql="REPLACE INTO multiplicationQuiz(email,score) VALUES('{$_SESSION['$myemail']}','$totalCorrectM')";
			...
			$sql="INSERT INTO multiplicationQuiz(email,score) VALUES('{$_SESSION['$myemail']}','$totalCorrectM')";
			...
			<div class="col-md-6">
				<h1>Final Quiz Grade</h1>
				<?php echo "<div id='results'>$totalCorrectM / 10 correct</div>";
				?>
				</br>
